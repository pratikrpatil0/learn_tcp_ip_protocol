	...
	while (1) {  // start an infinite loop to keep running this process continuously
		...
		if ((pkt.flags & FLAG_DATA) && pkt.len == 1) {
			...
			if (pkt.seq == expected_seq) {
				...
				printf("Accepted DATA seq=%u char='%c', ACK=%u\n", pkt.seq,  // print a human-readable log message to the console screen so we can see what is happening
					   (pkt.data >= 32 && pkt.data <= 126) ? pkt.data : '.', ack.ack_num);
			} else {
				ack.ack_num = expected_seq;
				printf("Out-of-order DATA seq=%u (expected %u), re-ACK=%u\n", pkt.seq, expected_seq, ack.ack_num);  // print a human-readable log message to the console screen so we can see what is happening
			}
