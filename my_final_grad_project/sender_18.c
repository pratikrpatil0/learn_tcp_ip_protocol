...
static int prepare_packets(Packet packets[MAX_PACKETS], const char *message, int len) {
    ...
    for (int i = 0; i < total_packets; i++) {
        ...
        packets[i].seq = (uint8_t)(1 + offset);
...
static int send_with_cc_sr_adaptive(int sockfd, const struct sockaddr_in *receiver_addr, const char *message, int len) {  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
    ...
    int recover_idx = -1;
    int peer_rwnd = WINDOW_SIZE * CHUNK_SIZE;
    long long last_probe_ms = 0;

    while (base < total_packets) {
        int cwnd_bytes = (int)cwnd * CHUNK_SIZE;
        if (cwnd_bytes < CHUNK_SIZE) {
            cwnd_bytes = CHUNK_SIZE;
        }
        if (cwnd_bytes > WINDOW_SIZE * CHUNK_SIZE) {
            cwnd_bytes = WINDOW_SIZE * CHUNK_SIZE;
        }
        if (peer_rwnd < 0) {
            peer_rwnd = 0;
        }
        if (peer_rwnd > WINDOW_SIZE * CHUNK_SIZE) {
            peer_rwnd = WINDOW_SIZE * CHUNK_SIZE;
        }
        ...
        if (ack_pkt.ack_num < 1 || ack_pkt.ack_num > (uint8_t)(len + 1)) {
        ...
        if (dup_ack_repeats == 3) {
            ...
            if (target_idx >= 0 && target_idx < total_packets && target_idx < next_to_send && !acked[target_idx]) {
                ...
                printf("Fast retransmit missing seq=%u, enter fast recovery (cwnd=%.2f ssthresh=%.2f)\n",  // print a human-readable log message to the console screen so we can see what is happening
...
static int close_with_fin(int sockfd, const struct sockaddr_in *receiver_addr, uint8_t fin_seq, SenderState *state) {  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
    Packet fin_pkt;
    Packet resp;
    ...
    *state = S_TIME_WAIT;
    ...
    while (now_ms() - tw_start < TIME_WAIT_MS) {
        ...
        if (resp.flags & FLAG_FIN) {
            ...
            ack.flags = FLAG_ACK;
            ack.ack_num = (uint8_t)(resp.seq + 1);
            finalize_packet(&ack);
...
int main(void) {  // the main entry point of the program where execution begins
    ...
    if (connect_handshake(sockfd, &receiver_addr, &state) != 0) {
    ...
    printf("Sending v18 state-machine+byte-stream+flow-control+persist+adaptive-RTO+cc (window=%d chunk=%d): \"%s\" (%d chars)\n\n", WINDOW_SIZE,  // print a human-readable log message to the console screen so we can see what is happening
