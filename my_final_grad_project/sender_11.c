...
static int send_with_selective_repeat(int sockfd, const struct sockaddr_in *receiver_addr, const char *message, int len) {
    ...
    while (base < total_packets) {
        while (next_to_send < total_packets && next_to_send < base + WINDOW_SIZE) {
            ...
            printf("Sent DATA pkt=%d seq=%u len=%u\n", next_to_send + 1, packets[next_to_send].seq,
                   packets[next_to_send].len);
            next_to_send++;
        }
        ...
        if (wait_for_response(sockfd, receiver_addr, &ack_pkt) != 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                for (int i = base; i < next_to_send; i++) {
                    ...
                    sent_at_ms[i] = now;
                    retransmitted++;
                    printf("Retransmitted DATA pkt=%d seq=%u (retry=%d)\n", i + 1, packets[i].seq, retries[i]);
                }
...
int main(void) {
    ...
    printf("Sending v11 SR+chunking (window=%d chunk=%d): \"%s\" (%d chars)\n\n", WINDOW_SIZE, CHUNK_SIZE, message,
           len);

    int total_packets = send_with_selective_repeat(sockfd, &receiver_addr, message, len);
    if (total_packets < 0) {
        close(sockfd);
        return 1;
    }
