...
static int send_with_cc_sr_adaptive(int sockfd, const struct sockaddr_in *receiver_addr, const char *message, int len) {  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
    ...
    while (base < total_packets) {
        ...
        if (flight_limit > peer_rwnd) {
            flight_limit = peer_rwnd;
        }
        ...
        if (ack_pkt.ack_num < 1 || ack_pkt.ack_num > (uint8_t)(total_packets + 1)) {
        ...
        if (ack_upto > base) {
            ...
            for (int i = base; i < ack_upto && i < total_packets; i++) {
                if (!acked[i]) {
                    acked[i] = 1;
            ...
            printf("Received cumulative ACK=%u (window base %d -> %d)\n", ack_pkt.ack_num, old_base + 1, ack_upto + 1);  // print a human-readable log message to the console screen so we can see what is happening
            ...
            if (in_fast_recovery && ack_upto > recover_idx) {
                in_fast_recovery = 0;
