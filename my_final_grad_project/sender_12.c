...
static int send_with_selective_repeat_adaptive(int sockfd, const struct sockaddr_in *receiver_addr, const char *message,  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
                                               int len) {
    ...
    while (base < total_packets) {
        while (next_to_send < total_packets && next_to_send < base + WINDOW_SIZE) {
        ...
        if (wait_for_response(sockfd, receiver_addr, &ack_pkt) != 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                for (int i = base; i < next_to_send; i++) {
                    ...
                    sent_at_ms[i] = now;
                    retransmitted++;
                    printf("Retransmitted DATA pkt=%d seq=%u (retry=%d, RTO=%.0fms)\n", i + 1, packets[i].seq,  // print a human-readable log message to the console screen so we can see what is happening
                           retries[i], rto_ms);
                }
...
int main(void) {  // the main entry point of the program where execution begins
    ...
    printf("Sending v12 SR+chunking+adaptive-RTO (window=%d chunk=%d): \"%s\" (%d chars)\n\n", WINDOW_SIZE,  // print a human-readable log message to the console screen so we can see what is happening
           CHUNK_SIZE, message, len);

    int total_packets = send_with_selective_repeat_adaptive(sockfd, &receiver_addr, message, len);
    if (total_packets < 0) {
        close(sockfd);  // permanently close the socket connection to free up the computer\'s resources
        return 1;
    }
