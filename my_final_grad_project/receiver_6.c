...
int main(){  // the main entry point of the program where execution begins
    ...
    while(1){  // start an infinite loop to keep running this process continuously
        ...
        if(received_parity != expected_parity){
            ...
            // Send NAK
            char nak = 0;
            sendto(sockfd, &nak, sizeof(nak), 0, (struct sockaddr *)&client_addr, addr_len);  // dispatch and send the data packet over the network to the assigned receiver address
            printf(" [Sent NAK]");  // print a human-readable log message to the console screen so we can see what is happening
        } else{
            ...
            printf(" [Sent ACK]");  // print a human-readable log message to the console screen so we can see what is happening

            // Ignore duplicates from sender retries, but still ACK them.
            if ((unsigned char)seq_num != (unsigned char)expected_seq) {
                printf(" [Duplicate/out-of-order seq, ignored]");  // print a human-readable log message to the console screen so we can see what is happening
                printf("\n");  // print a human-readable log message to the console screen so we can see what is happening
                continue;
            }
            ...
            if (char_count < (int)sizeof(received_message) - 1) {
                received_message[char_count] = (char)received_byte;
                char_count++;
                expected_seq++;
            } else {
                printf(" [ERROR: Message buffer full]\n");  // print a human-readable log message to the console screen so we can see what is happening
                break;
            }
        }
