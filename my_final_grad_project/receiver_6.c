...
int main(){
    ...
    while(1){
        ...
        printf("Receiving character %d [Seq: %d]: ", char_count + 1, seq_num);
        ...
        if(received_parity != expected_parity){
            ...
            // Send NAK
            char nak = 0;
            sendto(sockfd, &nak, sizeof(nak), 0, (struct sockaddr *)&client_addr, addr_len);
            printf(" [Sent NAK]");
        } else{
            printf(" [OK]");
            ...
            if ((unsigned char)seq_num != (unsigned char)expected_seq) {
                printf(" [Duplicate/out-of-order seq, ignored]");
                printf("\n");
                continue;
            }
