...
static uint8_t compute_advertised_rwnd(const int buffered[MAX_PACKETS + 2], uint8_t expected_seq) {
    int used = 0;
    int start = (int)expected_seq;
    ...
    return (uint8_t)(WINDOW_SIZE - used);
}
...
int main(void) {  // the main entry point of the program where execution begins
    ...
    while (1) {  // start an infinite loop to keep running this process continuously
        ...
        if (!checksum_valid(&pkt)) {
            ...
            dup_ack.ack_num = expected_seq;
            dup_ack.rwnd = compute_advertised_rwnd(buffered, expected_seq);
        ...
        if (pkt.flags & FLAG_FIN) {
            ...
            fin_ack.rwnd = compute_advertised_rwnd(buffered, expected_seq);
        ...
        if ((pkt.flags & FLAG_DATA) && pkt.len <= CHUNK_SIZE) {
            ...
            if (pkt.seq >= expected_seq && pkt.seq < (uint8_t)(expected_seq + WINDOW_SIZE)) {
                ...
                ack.rwnd = compute_advertised_rwnd(buffered, expected_seq);
            ...
            printf("Out-of-window DATA seq=%u (expected range %u-%u), ACK=%u rwnd=%u\n", pkt.seq, expected_seq,  // print a human-readable log message to the console screen so we can see what is happening
                   (uint8_t)(expected_seq + WINDOW_SIZE - 1), ack.ack_num, ack.rwnd);
        }
    }
