...
static int send_with_go_back_n(int sockfd, const struct sockaddr_in *receiver_addr, const char *message, int len) {  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
    ...
    while (base < len) {
        ...
        if (wait_for_response(sockfd, receiver_addr, &ack_pkt) != 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                timeout_retries++;
                printf("Timeout: retransmitting window [%d..%d]\n", base + 1, next_to_send);  // print a human-readable log message to the console screen so we can see what is happening
        ...
        if (ack_pkt.ack_num > current_base_ack && ack_pkt.ack_num <= max_sent_ack) {
            ...
            printf("Received stale/out-of-range ACK=%u (base=%d next=%d)\n", ack_pkt.ack_num, base + 1,  // print a human-readable log message to the console screen so we can see what is happening
                   next_to_send + 1);
        }
    }
