...
// FSM focus: READ_CHARACTER -> BUILD_BIT_FRAME -> SEND_FRAME -> WAIT_ACK_NAK -> RETRY_OR_ADVANCE -> TERMINATE
...
int main(){  // the main entry point of the program where execution begins
    ...
    for(int char_index = 0; char_index < message_length; char_index++){
        ...
        // wait for ACK/NAK
        char ack_buffer[1];
        recvfrom(sockfd, ack_buffer, sizeof(ack_buffer), 0, (struct sockaddr *)&from_addr, &addr_len);  // wait and listen patiently to receive an incoming packet from the network
    ...
    // wait for final ACK/NAK
    char ack_buffer[1];
    recvfrom(sockfd, ack_buffer, sizeof(ack_buffer), 0, (struct sockaddr *)&from_addr, &addr_len);  // wait and listen patiently to receive an incoming packet from the network
    printf("Termination signal sent | Received %s\n", ack_buffer[0] == 1 ? "ACK" : "NAK");  // print a human-readable log message to the console screen so we can see what is happening
