...
// FSM focus: LISTEN -> HANDSHAKE_CONFIRMED -> BUFFER_CHUNK_STREAM -> SEND_CUMULATIVE_ACK -> HOLD_DUPLICATE_ACK -> ACCEPT_FIN -> CLOSED
        ...
        if ((pkt.flags & FLAG_DATA) && pkt.len <= CHUNK_SIZE) {
            ...
            if (pkt.seq >= expected_seq && pkt.seq < (uint8_t)(expected_seq + WINDOW_SIZE)) {
                ...
                if (send_packet(sockfd, &peer_addr, peer_len, &ack) != 0) {
                    ...
                    printf("Sent cumulative ACK=%u\n", ack.ack_num);  // print a human-readable log message to the console screen so we can see what is happening
                }
                continue;
            }
