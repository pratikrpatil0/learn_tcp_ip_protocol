...
// FSM focus: LISTEN -> SYN_RECEIVED -> ESTABLISHED -> BUFFER_IN_WINDOW_DATA -> SEND_ACKS -> ACCEPT_FIN -> CLOSED
    ...
    while (1) {  // start an infinite loop to keep running this process continuously
        ...
        if ((pkt.flags & FLAG_DATA) && pkt.len == 1) {
            ...
            if (pkt.seq >= expected_seq && pkt.seq < (uint8_t)(expected_seq + WINDOW_SIZE)) {
                if (!buffered[pkt.seq]) {
                    buffered[pkt.seq] = 1;
                    reorder_buffer[pkt.seq] = (char)pkt.data;
                    printf("Buffered DATA seq=%u char='%c'\n", pkt.seq,  // print a human-readable log message to the console screen so we can see what is happening
                           (pkt.data >= 32 && pkt.data <= 126) ? pkt.data : '.');
                } else {
                ...
                while (expected_seq <= MAX_MSG_LEN && buffered[expected_seq]) {
                    if (count < MAX_MSG_LEN) {
                        received[count++] = reorder_buffer[expected_seq];
                    }
