...
// FSM focus: SYN_SENT -> ESTABLISHED -> SEND_CHUNK -> UPDATE_CWND_RTO -> FAST_RECOVERY -> FIN_SENT -> CLOSED
...
int main(void) {  // the main entry point of the program where execution begins
    ...
    printf("Sending v15 cumulative-ACK+chunking+adaptive-RTO+cc+fast-retransmit (window=%d chunk=%d): \"%s\" (%d chars)\n\n", WINDOW_SIZE,  // print a human-readable log message to the console screen so we can see what is happening
