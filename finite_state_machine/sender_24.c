...
// FSM focus: CLOSED -> SYN_SENT -> ESTABLISHED -> DATA_TRANSFER -> FIN_WAIT_1 -> FIN_WAIT_2 -> TIME_WAIT -> CLOSED
    ...
    for (int retry = 0; retry <= 5; retry++) {
        ...
        if ((resp.flags & (FLAG_SYN | FLAG_ACK)) == (FLAG_SYN | FLAG_ACK) && resp.ack_num == 1) {
            ...
            *negotiated_mss = peer_mss;
            ...
            if (resp.ts_val != 0) { ts_supported = 1; peer_ts_recent = resp.ts_val; }
...
static int send_with_cc_sr_adaptive(int sockfd, const struct sockaddr_in *receiver_addr, const char *message, int len, uint8_t mss, uint8_t wscale) {  // define a variable to hold our socket file descriptor, which acts as an ID for our network connection
    ...
    while (base < total_packets) {
        ...
        // --- SACK PROCESSING ---
        if ((ack_pkt.flags & FLAG_SACK) && ack_pkt.sack_count > 0) {
            for (int sb = 0; sb < ack_pkt.sack_count; sb++) {
                uint32_t left = ack_pkt.sack_blocks[sb].left_edge;
                uint32_t right = ack_pkt.sack_blocks[sb].right_edge;
                printf("[SACK] Block %d: [%u, %u)\n", sb, left, right);  // print a human-readable log message to the console screen so we can see what is happening

                for (int i = base; i < total_packets; i++) {
                    if (!acked[i]) {
                        uint32_t p_start = packets[i].seq;
                        uint32_t p_end = p_start + packets[i].len;
                        if (p_start >= left && p_end <= right) {
                            acked[i] = 1;
                            printf("[SACK] Selectively acked pkt=%d seq=%u\n", i + 1, p_start);  // print a human-readable log message to the console screen so we can see what is happening
                        }
                    }
                }
            }
        ...
        if ((int)ack_pkt.ack_num > last_cum_ack) {
            ...
            while (base < total_packets && acked[base]) {
                base++;
                 ...
                 printf("Received byte cumulative ACK=%" PRIu32 " (packet-base %d -> %d TS=%u ECR=%u)\n", ack_pkt.ack_num, old_base + 1,  // print a human-readable log message to the console screen so we can see what is happening
                   base + 1, ack_pkt.ts_val, ack_pkt.ts_ecr);

            if (ts_supported && ack_pkt.ts_ecr != 0) {
                double sample_ms = (double)(now_ts32() - ack_pkt.ts_ecr);
                if (sample_ms < 1.0) {
                    sample_ms = 1.0;
                }
