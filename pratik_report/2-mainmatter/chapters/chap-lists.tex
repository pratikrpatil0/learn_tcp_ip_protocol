...
In this chapter, we will discuss the complete version to version protocol
evolution. The Strategy we will implement is incremental. Each version of the
protocol retains all of the capabilities from prior versions, however each new
version introduces at least one carefully selected feature. Thus, this method
provides increased source correctness and also provides a greater practical
capability to debug individual implementations.

\section{Global Design Principles}
The same design principles are followed across all versions.

\begin{enumerate}
    \item \textbf{One focus at a time:} each version teaches one protocol idea.
    \item \textbf{Clear state:} sender and receiver keep simple names for
    sequence numbers, timeouts, and window state.
    \item \textbf{Predictable moves:} each packet leads to a known state
    change.
    \item \textbf{Easy to trace:} console output makes each test run easier to
    follow.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item point it to the receiver,
    \item send the datagram,
    \item receive the datagram.
\end{enumerate}
...
\caption{Version 0: one bit sent over UDP.
}
...
Educational value:
\begin{enumerate}
    \item seeing how binary values are stored,
    \item learning why bit order matters,
    \item rebuilding the byte at the receiver.
\end{enumerate}
...
sent at the end to flag message completion. At this stage, communication works
...
In version 9, an increase in efficiency has been implemented using a "sender
window" (4 packets typically), where the sender can send multiple packets
before receiving an ACK. If the sender does not receive an ACK after a
timeout, it will re-send packets from the base. The receiver will also use a
cumulative ACK-style mechanism to acknowledge the last-sequence number
...
missing sequence arrives. This avoids unnecessary retransmission of
...
Version 12 introduces RTT sampling and adaptive RTO calculation. The fixed timeout mechanism does not perform well when the delay between sending and receiving packets is variable. The adaptive timeout mechanism adapts the retransmit threshold dynamically, thereby reducing both premature retransmissions and prolonged idle periods before initiating a retransmission.
...
The latter versions advance the transport process into a more sophisticated and educational TCP-like environment. This family emphasizes RTT-sensitive refinement of timeouts, state awareness based on timestamps, SACK-like reasoning, avoiding congestion, and improved recovery procedure. The visualizer uses later versions of the project to demonstrate on a step by step basis how the FSM and UML timeline can assist in explaining protocol decisions.

The RFCs and texts that describe TCP in practice provide the main design inspiration for this family. The RFC 6298 describes the retransmission timer, RFC 5681 describes congestion avoidance, RFC 2018 provides options for selective acknowledgment (SACK). TCP extensions for high speed are found in RFC 7323, and RFC 793 provides the core TCP model. Additionally, the Stevens, Kurose and Ross, and Tanenbaum and Wetherall texts (used for the implementation style) make it easier to relate TCP theory to the actual sender and receiver code structure used in the project Passport et al. (2011); Allman et al. (2009); Mathis et al. (1996); Borman et al. (2014); Postel (1981); Stevens and Wright (1995); Kurose and Ross (2021); Tanenbaum and Wetherall (2011).

This family is very useful to the report because it demonstrates the moment when the learning project approaches real TCP thought processes both visually and conceptually.
...
to an integrated TCP-inspired system. This illustrates how the use of timing, selective feedback, congestion awareness and systematic recovery can be added in layers while maintaining traceability for learners.

\section{Chapter Summary}
This chapter described the stage changes of the entire project starting from merely a UDP demo until eventually evolving into a comprehensive transport protocol lab. The following chapter details the mathematical algorithms and equations used to define the process of implementation from a quantitative perspective. Finally, Chapter 4 will conclude with all of the empirical data, analysis and visual timeline findings to finalize the educational narrative.